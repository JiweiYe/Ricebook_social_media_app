...
var redirectUrl = 'https://mighty-reef-86521.herokuapp.com/login/facebook'
...
export function register({username, displayname, email, phone, birth, zipcode, password, pwconf}) {
    return (dispatch) => {
        ...
        .then((response) => {
            return dispatch(updateSuccess(`Success! You have registered as "${response.username}".`))
        }).catch((err) => {
            return dispatch(updateError("Error! This username maight already be registered"))
        })
    }
}
